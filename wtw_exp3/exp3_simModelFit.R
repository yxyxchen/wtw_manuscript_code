simModelFit = function(encodeModel, decodeModel, isFirstFit,  batchIdx = NULL, parallel = F){
  ...
  dir.create("../../genData/wtw_exp3/simModelFit")
  dir.create("stanWarnings")
  dir.create(sprintf("../../genData/wtw_exp3/simModelFit/%s", encodeModel))
  dir.create(sprintf("../../genData/wtw_exp3/simModelFit/%s/%s", encodeModel, decodeModel))
  ...
  load(sprintf("../../genData/wtw_exp3/simulation/%s.RData", encodeModel))
  
  # prepare inputs
  outputDir = sprintf("../../genData/wtw_exp3/simModelFit/%s/%s", encodeModel, decodeModel)
  if(isFirstFit){
    config = list(
      nChain = 4,
      nIter = 8000,
    ...
    if(!is.null(batchIdx)){
      if(batchIdx == 1){
        ...
        trialData = trialData[21 : length(trialData)]
      }
    }
  }
