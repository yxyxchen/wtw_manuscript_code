paraRecovery = function(){
  ...
  simPara = loadExpPara(paraNames, sprintf("../../genData/wtw_exp3/simModelFit/%s/%s", modelName, modelName)) 
  ...
  for(i in 1 : nPara){
    ...
    junk = sprintf("r[tau] = %.2f", rankTaus[[i]])
    figs[[i]] = plotData %>% ggplot(aes(truth, recovered)) + geom_point(color = "#636363") +
      ...
      ylim(c(lowLim, upLim)) +
      coord_fixed(ratio = 1)
    # ggsave(file.path("..", "figures", "exp3", "paraRecovery", sprintf("%s.eps", paraName)), figs[[i]], width = 4, height = 4)
  }
