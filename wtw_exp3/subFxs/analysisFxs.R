...
trialPlots <- function(thisTrialData) {
  ...
  thisTrialData$rewarded = thisTrialData$trialEarnings != 0
  p = thisTrialData %>%
    ggplot(aes(trialNum, timeWaited,color = factor(rewarded))) +
    ...
    geom_point(data = thisTrialData[thisTrialData$trialEarnings == 0, ],
               ...
               color = 'black', size = 4) +
    geom_point(data = thisTrialData[thisTrialData$trialEarnings == -1,], aes(trialNum, timeWaited), color = "red") +
    xlab("Trial") + ylab("Time (s)")  + myTheme 
  print(p)
  return(p)
}
