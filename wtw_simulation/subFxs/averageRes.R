...
averageRes = function(sim_, recorded_timepoints){
  # constants 
  ...
  nRecord = length(lower_boundaries) # number of analysis time windows 
  ...
  for(i in 1 : nRep){
    for(j in 1 : nRecord){
      ...
      # sub_auc_reps[j,i] = kmsc(sim[lower_boundaries[j] < sim$sellTime & sim$sellTime <= recorded_timepoints[j],], min(delayMaxs), F, kmGrid)$auc
    }
  }
