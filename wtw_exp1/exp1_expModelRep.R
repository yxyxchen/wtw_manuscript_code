...
expModelRep = function(modelName, allData = NULL, MFResults = NULL, repOutputs = NULL){
  ...
  r2_ = vector(length = length(tGrid))
  auc_r2_ = vector(length = length(tGrid))
  ...
  for(tIdx in 1 : length(tGrid)){
    ...
    r2_[tIdx] = summary(lm(y ~ x))$r.squared
    auc_r2_[tIdx] = summary(lm(y ~ auc))$r.squared
  }
  sum(r2_ * ss_) / sum(ss_)
  mean(auc_r2_)
  data.frame(
    ...
    auc_r2 = auc_r2_,
    time = tGrid
  ) %>% ggplot(aes(time, r2)) + geom_line() +
    geom_line(aes(time, auc_r2), inherit.aes = F, color = "red")
  # # + geom_line(aes(time, explained), inherit.aes = F)
  # # still lower than PCA though; this problem is ill-defined for PCA though...
  # pca_res <- prcomp(timeWTW_[sumStats$condition == condition & passCheck, ], center = TRUE, scale = FALSE) # I am not sures
  # summary(pca_res)
